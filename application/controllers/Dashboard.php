       ...
       // $limit = 10;
		...
		$year			     		 = date("Y");
			for($i=0; $i<count($area); $i++){
				$temp 							= $area[$i];
				$key 							= $temp['id'];
				$databersih 					= $this->m_hitung->get_data_bersih_area($key, $month, $year);
				$datakotor	 					= $this->m_hitung->get_data_kotor_area($key, $month, $year);
				
				$temp['databersih']				= $databersih['jum'];
				$temp['datakotor']				= $datakotor['jum'];
				$area[$i]			   			= $temp;
			}
		
		$data['title'] = 'Dashboard';
		$data['area'] = $area;
		$data['month'] = get_month($month);
        ...
        $this->load->view('templates/topbar',$data);
        $this->load->view('templates/sidebar');
		$this->load->view('dashboard/hasil_area',$data);
		$this->load->view('templates/layout_footer');
        $this->load->view('templates/footer');
    }
	
	public function hasil_subarea($id){
		$subarea						 = $this->m_area->get_all_sub($id);
		$month			     		 = "08";
		//$month			     		 = date("m");
		$year			     		 = date("Y");
			for($i=0; $i<count($subarea); $i++){
				$temp 							= $subarea[$i];
				$key 							= $temp['id'];
				$databersih 					= $this->m_hitung->get_data_bersih_subarea($key, $month, $year);
				$datakotor	 					= $this->m_hitung->get_data_kotor_subarea($key, $month, $year);
		...
		$data['month'] = get_month($month);
		$data['year'] = get_year($year);
		$this->load->view('dashboard/hasil_subarea',$data);
	}
