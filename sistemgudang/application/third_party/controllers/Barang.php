...
//dumper($subdata['lokasi']);
    ...
    public function transaksi(){
        ...
        // die();
        $ids = array();
        if($this->session->userdata('transaksi') != array()){
            foreach ($this->session->userdata('transaksi') as $id_out) {
                array_push($ids,$id_out['id_stock']);
            };
            $ids = implode(", ", $ids);
            ...
            if($this->m_stock->insert_resi_final($data_resi)){
                $this->session->set_userdata('temp_transaksi', $this->session->userdata('transaksi'));
                $this->session->set_userdata('transaksi',array());
            }
        ...
        // var_dump($this->session->userdata('temp_transaksi'));
        render_view('template', $data);
    }
    ...
    public function delete_item_transaksi(){
     ...
     public function resi_print(){
        ...
        $no_resi = $resi['id'];
        $barang = $this->session->userdata('temp_transaksi');
        $profile                    = get_profile();
        $subdata['barang']          = $barang;
        $subdata['user']            = userid();
